...
def test_cancelling_vlm_run_invalidates_inflight_result(
    database_url: str,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    client, model_version_id, _sample_id = _trained_toy_context(database_url, tmp_path, monkeypatch)
    inferred = _run_forced_ood_inference(client, model_version_id, tmp_path)
    assert inferred.status_code == 200
    review_id = inferred.json()["inference_result"]["review_item_id"]
    engine = create_engine(database_url)
    with engine.begin() as conn:
        review_db_id = conn.execute(
            sa.select(review_items.c.id, review_items.c.inference_event_id).where(
                review_items.c.review_key == review_id
            )
        ).mappings().one()
        conn.execute(
            inference_events.update()
            .where(inference_events.c.id == review_db_id["inference_event_id"])
            .values(decision="abstain")
        )
        conn.execute(
            review_items.update()
            .where(review_items.c.id == review_db_id["id"])
            .values(risk_type="low_confidence")
        )

    created = client.post("/api/vlm-review-runs", json={"mode": "assisted", "limit": 1})
    assert created.status_code == 202
    run_id = created.json()["vlm_review_run"]["run_id"]
    duplicate = client.post("/api/vlm-review-runs", json={"mode": "assisted", "limit": 1})
    assert duplicate.status_code == 409
    store = client.app.state.vlm_review_store
    claimed = store.claim_next_result()
    assert claimed is not None
    assert store.is_result_active(claimed.result_id) is True

    cancelled = client.post(f"/api/vlm-review-runs/{run_id}/cancel")
    assert cancelled.status_code == 200
    assert store.is_result_active(claimed.result_id) is False
    review_store = DatabaseReviewStore(create_engine(database_url))
    with pytest.raises(RuntimeError, match="no longer active"):
        review_store.complete_review(
            ...
            final_label=claimed.candidate_labels[0],
            reviewer="vlm:Fine-R1-3B",
    ...
    with pytest.raises(RuntimeError, match="no longer active"):
        store.complete_result(
            claimed.result_id,
            suggested_label=claimed.candidate_labels[0],
            reasoning="cancelled task must not commit",
            raw_output="<answer>cancelled</answer>",
            image_sha256="0" * 64,
    ...
    result = store.fail_result(claimed.result_id, "late worker error")
