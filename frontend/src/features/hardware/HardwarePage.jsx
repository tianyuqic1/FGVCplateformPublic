...
export function HardwarePage() {
  ...
  return <div className="fv-feature-page hw-page">
    <PageHeading eyebrow="Compute resources" title="硬件监控" description="查看训练与推理节点的资源使用情况，关注计算余量与持续的资源压力。" actions={<span className="hw-scope"><Icon name="RadioTower" size={16} />宿主机资源</span>} />
    ...
    {!snapshot ? <Panel title="计算节点">{state.loading ? <EmptyState icon="LoaderCircle" title="正在读取硬件状态" description="首次采集后将显示节点和资源曲线。" /> : <EmptyState icon="RadioTower" title={nodeId ? "暂时无法读取此节点" : "尚无计算节点接入"} description={nodeId ? "可切换其他节点，或等待采集服务恢复后重试。" : "启动部署中的 hardware-collector 后，节点会自动出现在这里。"} />}</Panel> : <>
      ...
      <Panel eyebrow="Device inventory" title="GPU 设备" aside={<span className="hw-caption">NVIDIA · {snapshot.gpus.length} 张可见设备</span>}>
      ...
      <div className="hw-chart-grid"><Panel eyebrow="Resource history" title="GPU 与显存趋势" aside={<span className="hw-caption">实线：利用率 · 虚线：显存</span>}>{gpuOption.series.length ? <EChart group={chartGroup} option={gpuOption} ariaLabel="GPU 与显存占用趋势" /> : <EmptyState icon="LineChart" title="暂无 GPU 历史数据" description="接入可采集的 GPU 后开始记录。" />}</Panel><Panel eyebrow="Resource history" title="CPU 与内存趋势" aside={<span className="hw-caption">{window === "24h" ? "每 5 分钟取末次读数" : window === "1h" ? "每 15 秒取末次读数" : "原始采样"}</span>}><EChart group={chartGroup} option={hostOption} ariaLabel="CPU 与内存使用趋势" /></Panel></div>
      <Panel eyebrow="Storage capacity" title="存储空间" aside={<span className="hw-caption">按文件系统去重 · GiB</span>}>
      ...
      <Panel eyebrow="Active workloads" title="运行中的训练任务" aside={<span className="hw-caption">{data.tasks.length} 个 · 节点级关联</span>}>
      ...
      <Panel eyebrow="Resource notices" title="资源提醒" aside={<span className="hw-caption">连续 60 秒超阈值才触发</span>}>
