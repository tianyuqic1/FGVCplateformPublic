        ...
        {jobs.map((job) => (
          <div className="timeline-item" key={job.id}>
            <div>
              ...
              <div className="row-meta">{job.dataset_version_key}</div>
            ...
            {job.status === "succeeded" && <Link className="ghost-button" to={`/datasets/${encodeURIComponent(job.dataset_id)}`}>打开数据集</Link>}
          </div>
        ))}
      </div>
    </Panel>
