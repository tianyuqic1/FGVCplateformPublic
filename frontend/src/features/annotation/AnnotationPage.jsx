...
export function AnnotationPage() {
  ...
  const [worker, setWorker] = useState({ online: null });
  ...
  useEffect(() => {
    let stopped = false;
    const refresh = async () => {
      const [content, status] = await Promise.allSettled([Promise.all([reload(), loadProjects()]), getAnnotationStatus()]);
      if (stopped) return;
      setWorker(status.status === "fulfilled" ? status.value : { online: null });
      if (content.status === "rejected") setError(content.reason.message);
    };
    refresh();
  ...
  return <div className="annotation-page">
    <header className="ann-hero"><div><h1>AI 辅助标注</h1><p>AI 推荐候选类别，由人工确认最终标签。成功加入检索库的样本可用于后续分析。</p></div>
      <span className={`ann-service ${worker.online ? "online" : ""}`} role="status"><i />{annotationServiceLabel(worker)}</span></header>
    ...
    <section className="ann-project-bar">
      ...
      {project && <div className="ann-project-stats"><span><strong>{project.total}</strong> 张图片</span><span><strong>{project.confirmed}</strong> 已确认</span><span><strong>{project.indexed}</strong> 已加入检索库</span></div>}
    ...
    {creating && <form className="ann-create" onSubmit={create}>
      <div><h2>新建标注项目</h2><p>项目内独立保存类别、标签和检索参考。类别表创建后固定，避免不同项目的类别混用。</p>
    ...
    {project && <section className="ann-workspace">
      <aside className="ann-queue"><div className="ann-queue-head"><h2>图片列表 <small>{view.total}</small></h2><PaginatedSelect aria-label="标注状态" value={filter} onChange={e => { setFilter(e.target.value); setPage(1); }} options={[{ value: "", label: "全部状态" }, ...Object.entries(states).map(([value, label]) => ({ value, label }))]} /></div>
        <div className="ann-thumbnails">{view.items.map(task => <button key={task.id} className={`ann-thumb ${selectedId === task.id ? "active" : ""}`} onClick={() => setSelectedId(task.id)} aria-pressed={selectedId === task.id}>
          <img src={picture(task.id)} alt="待标注样本缩略图" loading="lazy" /><span><strong title={task.filename}>{task.filename}</strong><small className={`ann-state ${task.status}`}>{states[task.status]}</small>{task.status === "confirmed" && <small>{names[task.label]} · {task.indexed ? "已加入检索库" : "等待加入检索库"}</small>}</span>
      ...
      <main className="ann-canvas"><div className="ann-canvas-head"><div><h2>{selected ? selected.filename : "选择一张图片"}</h2></div>{selected && <span className={`ann-state ${selected.status}`}>{states[selected.status]}</span>}</div>
      ...
      <aside className="ann-inspector"><div className="ann-inspector-head"><h2>AI 推荐类别 · Top-{Math.min(10, project.classes.length)}</h2><p>候选按相对支持度排序，不是置信概率。</p></div>
