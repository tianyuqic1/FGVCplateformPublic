...
与 [Worker 状态页](worker-status.md) 分工：本页观察计算节点的资源，`/workers` 观察执行进程的心跳、就绪与负载。同一节点可运行多个 Worker，只有明确相同的 `FINEVISION_NODE_ID` 才关联；不能把整机 GPU 占用当作某个 Worker 的独占用量。两页均供管理员与业务人员只读使用，标注员不开放入口。
