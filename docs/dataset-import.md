...
- 保留当前 100000 张、5 GiB 总图片大小、32 MiB 单图限制。multipart 另有 128 MiB 头部额度；ZIP 使用 Store 模式，避免重复压缩 JPEG/PNG。
- 上传阶段最多等待 30 分钟；后台导入有独立的 20 分钟执行时限。

## 中断与恢复

上传期间的“停止上传”只中断浏览器请求。如果服务端已经提交任务，任务仍会继续，页面通过轮询展示真实结果。收到 `202` 后可以切换页面或刷新，文件无需保留在浏览器内存。

队列使用 2 分钟租约，每 30 秒续租。服务停止后任务保留在数据库，租约到期后可由新消费者领取。每次领取递增 attempt；旧消费者不能覆盖新 attempt 的结果。任务 ID 同时作为不可变版本 UUID，恢复使用相同 request_id 与内容指纹，已提交的注册结果直接复用，不会重复创建版本。

上传表单沿用当前 `name`（1–120 字）与 `request_id`（UUID），不再使用旧版手填 dataset_id/version_key。后台自动分配数据集唯一标识并创建 v1；同一请求正在处理或已经完成时拒绝重复入队，失败请求可重新上传。已有数据集的扩充继续使用 `/api/datasets/{dataset_id}/versions` 和现有自动递增版本链路，本次不改为异步。完成任务的目标数据集标识从 `job.result.dataset.dataset_id` 读取，任务名称为 `job.name`。

## 部署

先执行新增 Alembic 迁移 `20260910_0017`（接续模型发布的 `0016`），再启动更新后的 Go、Python compute 与前端。沿用当前部署的数据库和 MinIO 配置，无需新增 RabbitMQ 队列。
