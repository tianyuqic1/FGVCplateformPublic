...
type ImportJob struct {
	...
	VersionKey string              `json:"dataset_version_key"`
...
func (q *ImportQueue) Enqueue(ctx context.Context, key, versionKey, archivePath string) (ImportJob, error) {
	job := ImportJob{ID: uuid.NewString(), DatasetID: key, VersionKey: versionKey, Status: "queued", CreatedAt: time.Now().UTC()}
...
func (q *ImportQueue) ProcessNext(ctx context.Context) error {
	...
	result, importErr := q.Service.ImportArchive(workCtx, job.DatasetID, job.VersionKey, job.ID, job.Archive)
	cancel()
	<-heartbeatDone
