...
func (r *DatasetImportQueue) Enqueue(ctx context.Context, j dataset.ImportJob) error {
	err := pgx.BeginFunc(ctx, r.Pool, func(tx pgx.Tx) error {
		...
		var exists bool
		if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM dataset_versions v JOIN datasets d ON d.id=v.dataset_id WHERE d.dataset_key=$1 AND v.version_key=$2)`, j.DatasetID, j.VersionKey).Scan(&exists); err != nil {
			return err
		}
		if exists {
		...
		_, err = tx.Exec(ctx, `INSERT INTO dataset_import_jobs(id,dataset_key,version_key,archive,status,created_at) VALUES($1,$2,$3,$4,'queued',$5)`, j.ID, j.DatasetID, j.VersionKey, archive, j.CreatedAt)
		return err
	})
...
const importJobColumns = `id::text,dataset_key,version_key,archive,status,attempt,created_at,result,error`

func scanImportJob(row pgx.Row) (dataset.ImportJob, error) {
	var j dataset.ImportJob
	var archive, result []byte
	err := row.Scan(&j.ID, &j.DatasetID, &j.VersionKey, &archive, &j.Status, &j.Attempt, &j.CreatedAt, &result, &j.Error)
	if err != nil {
		return j, err
	}
