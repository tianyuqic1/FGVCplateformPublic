...
func TestDatasetImportQueueDurabilityBoundsRecoveryAndFencing(t *testing.T) {
	...
	newJob := func() dataset.ImportJob {
		id := uuid.NewString()
		return dataset.ImportJob{ID: id, Name: "test-" + id, RequestID: uuid.NewString(), Status: "queued", CreatedAt: time.Now().UTC(), Archive: artifact.Descriptor{URI: "s3://test/archive", ArtifactID: uuid.NewString(), DatasetVersionID: id}}
	}
	first := newJob()
	...
	if err = reopened.Finish(ctx, recovered, map[string]any{"dataset": map[string]any{"id": first.Name}}, ""); err != nil {
		t.Fatal(err)
	}
	...
	retry := newJob()
	retry.Name = next.Name
	retry.RequestID = next.RequestID
...
func TestDatasetImportRegistrationCanBeAcknowledgedAfterCrash(t *testing.T) {
	...
	publication := dataset.Publication{DatasetID: uuid.NewString(), DatasetKey: key, Name: "recovery", VersionID: id, Number: 1, RequestID: uuid.NewString(), Fingerprint: strings.Repeat("c", 64), SourceType: "initial_import", Archive: archive, ManifestArtifact: manifest, Manifest: data, Changes: map[string]any{}}
	encoded, _ := json.Marshal([]any{"", "", publication.Name, archive.SHA256, []string{}})
	digest := sha256.Sum256(encoded)
	publication.Fingerprint = hex.EncodeToString(digest[:])
	if _, err = repo.Save(ctx, publication); err != nil {
		t.Fatal(err)
	}
	// No scanner or store: recovery must reuse the committed publication before
	// attempting compute or creating another artifact/version.
	service := &dataset.Service{Repository: repo}
	replayed, err := service.ImportArchive(ctx, publication.Name, publication.RequestID, id, archive)
	if err != nil || replayed == nil {
		t.Fatalf("replayed import: %v", err)
	}
	if replayed["version"].(map[string]any)["dataset_version_id"] != id {
		t.Fatalf("replay changed version: %v", replayed)
	}
	if _, err = repo.Save(ctx, publication); err != nil {
		t.Fatalf("concurrent commit replay: %v", err)
	}
	if _, err = repo.FindPublication(ctx, publication.RequestID, strings.Repeat("d", 64)); !errors.Is(err, dataset.ErrConflict) {
