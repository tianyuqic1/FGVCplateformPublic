...
func TestBackgroundImportFailureIsPersisted(t *testing.T) {
	...
	if _, err := queue.Enqueue(context.Background(), "example", uuid.NewString(), path); err != nil {
		t.Fatal(err)
	}
	if err := queue.ProcessNext(context.Background()); err != nil {
		t.Fatal(err)
...
func TestQueueRejectsInvalidImportBeforeStoring(t *testing.T) {
	// A nil service proves validation happens before object storage is called.
	q := &dataset.ImportQueue{}
	for _, input := range [][3]string{{"", uuid.NewString(), "archive"}, {"birds", "not-a-uuid", "archive"}, {"birds", uuid.NewString(), ""}} {
		if _, err := q.Enqueue(context.Background(), input[0], input[1], input[2]); err == nil {
			t.Fatalf("invalid import accepted: %v", input)
		}
	}
}
