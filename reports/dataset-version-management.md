...
按文件内容 SHA-256 去重，包含与评测图片的精确重复；同图不同标签拒绝发布。所选数据全部重复时不创建空版本。完整快照上限：100000 张、5 GiB，单图 32 MiB，与导入限制一致。原始图片字节去重不等于近似图片识别；新增类别仍受现有 readiness 检查，并不会自动为它补充验证或测试样本。
